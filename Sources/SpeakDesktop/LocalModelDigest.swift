...
/// Creates SHA-256 hashers for verifying downloaded model files.
///
/// This package contains no digest implementation of its own: Windows supplies
/// CNG (BCrypt) through its platform adapter and Apple platforms use CryptoKit.
/// A host without an audited provider cannot verify downloads and therefore
/// does not offer them.
public struct LocalModelDigestProvider: Sendable {
    ...
    /// The SHA-256 of `data`, for small in-memory values.
    ...
    /// Streams a file through a new hasher in bounded chunks, checking for
    /// cancellation between chunks.
    public func sha256(ofFileAt url: URL, chunkSize: Int = 4 << 20) throws -> String {
        let hasher = try makeSHA256()
