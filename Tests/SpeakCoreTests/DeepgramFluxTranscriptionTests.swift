...
/// Deepgram Flux (`v2/listen`) lives in the shared `DeepgramLiveClient`
/// alongside the classic `v1/listen` models, so both platforms speak the same
/// endpoints and parse the same events.
final class DeepgramFluxTranscriptionTests: XCTestCase {
    ...
    func testWebSocketURL_fluxUsesV2EndpointAndFluxParameters() throws {
        let url = try XCTUnwrap(
            DeepgramLiveClient.webSocketURL(
    ...
    func testWebSocketURL_novaKeepsV1EndpointAndLanguageParameter() throws {
        let url = try XCTUnwrap(
            DeepgramLiveClient.webSocketURL(model: "nova-3", language: "en_GB", sampleRate: 16_000)
        )
        let components = try XCTUnwrap(URLComponents(url: url, resolvingAgainstBaseURL: false))
        let query = Dictionary(
            uniqueKeysWithValues: try XCTUnwrap(components.queryItems).map { ($0.name, $0.value ?? "") }
    ...
    func testTranscriptEvent_fluxUpdateProducesPartial() {
        ...
        let event = DeepgramLiveClient.transcriptEvent(from: json, model: "flux-general-en")
    ...
    func testTranscriptEvent_novaResponseStillParses() {
        ...
        let event = DeepgramLiveClient.transcriptEvent(from: json, model: "nova-3")
