...
- If all the other six approval labels (`planning:product-approved`, `planning:security-approved`, `planning:performance-approved`, `planning:architecture-approved`, `planning:reliability-approved`, `planning:design-approved`) are already present, also add `planning:ready-for-dev` and remove `planning:in-discussion`.
