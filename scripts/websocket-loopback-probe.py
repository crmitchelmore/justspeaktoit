...
class ProbeHandler(socketserver.BaseRequestHandler):
    ...
    def handshake(self):
        ...
        # credentials, query strings, arbitrary request headers or WebSocket
        # keys enter logs.
    ...
    def run_gladia(self, route, scenario):
        ...
        for _ in range(GLADIA_MAX_MESSAGES):
            opcode, payload = self.read_gladia_message(route)
            if opcode is None:
                return
            if opcode == 2:
                if bytes(payload) != gladia_pcm(frames):
                ...
                if self.reply_while_streaming(scenario, frames):
                    return
                continue
            if opcode != 1 or json.loads(bytes(payload).decode("utf-8")) != {"type": "stop_recording"}:
    ...
    def finish_gladia(self, route, scenario, frames):
        if scenario == "gladia-hold":
            ...
            for _ in range(GLADIA_MAX_MESSAGES):
                if self.read_gladia_message(route)[0] is None:
                    return
            raise ValueError("gladia message count exceeded bound")
    ...
    def read_gladia_message(self, route):
        message = bytearray()
        message_opcode = None
        while True:
            final, opcode, payload = self.read_frame()
