...
root = Path.home() / 'Library/MobileDevice/Provisioning Profiles'
surface = sys.argv[1]
...
for name, identifier in profiles:
    ...
    if surface == 'ios' and os.environ['IOS_APP_GROUP'] not in entitlements.get('com.apple.security.application-groups', []):
    ...
    if name in ['ios-appstore', 'mac-app-store']:
        ...
        if cloud not in entitlements.get('com.apple.developer.icloud-container-identifiers', []):
