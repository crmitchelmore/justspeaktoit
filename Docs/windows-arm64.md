...
| From image 20260920.164.1 the label also serves `windows-11-vs2026-arm64`: Visual Studio 2026 with MSVC 14.51, whose STL stops any clang before 20 with STL1000. Swift 6.2.3 compiles C++ with its own clang 19, so `Package.swift` defines `_ALLOW_COMPILER_AND_STL_VERSION_MISMATCH` for the two Windows C++ targets, which use only C++17 library features | Run 36120606314 (native job log) |
