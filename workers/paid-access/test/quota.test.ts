...
describe('quota release and finalisation', () => {
  ...
  it('reclaims a reservation whose lease has expired', async () => {
    const quota = client({ monthlyAudioSeconds: 50, leaseSeconds: 30 });
    const userId = newUser();
    await quota.reserve({
      userId,
      period: PERIOD,
    ...
    // A client that crashes without finalising must not hold the allowance forever.
