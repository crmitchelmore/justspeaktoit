...
description: Security reviewer for PR plan-review discussions
on:
  pull_request:
...
---
# Security PR Plan Reviewer

Review the relevant pull request plan-review conversation for `${{ github.repository }}` from the `Security` lens.
...
- Pending: `plan-review:needs-security`
- Approved: `plan-review:security-approved`
...
7. Evaluate the pull request using this role's lens:
   - whether the diff introduces, removes, or weakens trust boundaries described in the linked plan
   - secrets handling, input validation, auth or authz changes, and whether logs or errors leak more than the plan allows
   - whether tests or verification evidence prove the safe path rather than only the happy path
   - whether any deviation from the approved plan expands privilege, exposure, or abuse surface without explicit sign-off
...
- When another role suggests implementation shortcuts that change trust boundaries, auth, or data handling, respond directly and explain the minimum safe shape that keeps the PR aligned with the approved plan.
...
- Add or keep `plan-review:needs-security`.
- Remove `plan-review:security-approved` if present.
