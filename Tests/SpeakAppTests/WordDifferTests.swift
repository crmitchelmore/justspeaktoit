...
final class WordDifferTests: XCTestCase {
    ...
    //   3. lcsHeavyPath – forces buildLCSTable on a 9x4 grid (O(n*m) with lowercased() in inner loop)
    ...
    /// 9x4 grid, exercising the O(n*m) inner loop and repeated `.lowercased()` calls.
