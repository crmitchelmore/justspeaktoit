...
| `licenses/` | Swift licence and ICU licence pinned to release-tag commits; the app's MIT licence; a Microsoft runtime notice | Redistribution terms and attribution |
...
non-system module from outside the bundle. In CI the closure is additionally
...
- **Swift runtime.** The swift.org installer is pinned by URL, SHA-256 and byte
  ...
  and refuses a DLL whose size differs from the package layout. Licence:
...
2. A new `bundle-execute` job on `windows-2022` runs **without any Swift setup
   ...
   - sets `PATH` to the Windows system directories only, clears Swift-related
     variables, confirms no `swift.exe` is reachable and that no bundled Swift
   ...
   - runs a **negative control**: `SpeakWindows.exe` with its resources but
     ...
     and fails the job;
   - runs `--self-test` and `--ui-smoke-test` from the bundle directory while
...
- The 34 unit tests pass locally with Python 3.14: PE import and version
