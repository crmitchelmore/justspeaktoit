...
extension DesktopHostController {
    ...
    package func retryHistory(_ identifier: String, onDeviceReadiness: (@Sendable (String) -> String?)? = nil) async {
        guard canUseHistory, let id = UUID(uuidString: identifier),
    ...
    package func retryRefusal(
        for model: String, onDeviceReadiness: (@Sendable (String) -> String?)? = nil
    ) -> String? {
        switch DesktopHistoryRetry.route(for: model) {
        case .remote: return nil
        case .onDevice:
            if let onDeviceReadiness { return onDeviceReadiness(model) }
            return Platform.localReadiness(model, controller: self)
        case .liveOnly:
