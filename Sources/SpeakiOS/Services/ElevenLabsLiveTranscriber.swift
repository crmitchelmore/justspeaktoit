...
/// iOS ElevenLabs live transcriber that integrates with AudioSessionManager.
...
public final class ElevenLabsLiveTranscriber: ObservableObject {
    ...
    public var modelID: String = "scribe_v1"
    ...
    private var elevenLabsClient: ElevenLabsLiveClient?
    ...
    /// Configure the API key for ElevenLabs.
    ...
    /// Start live transcription with ElevenLabs.
    public func start() async throws {
        guard !isRunning else { return }

        SpeakLogger.logTranscription(event: "start", model: "elevenlabs/\(modelID)")

        guard let apiKey, !apiKey.isEmpty else {
            let error = ElevenLabsLiveError.missingAPIKey
            SpeakLogger.logError(error, context: "ElevenLabsLiveTranscriber.start", logger: SpeakLogger.transcription)
            self.error = error
            throw error
        }
        ...
        print("[ElevenLabsLiveTranscriber] Started")
    }
    ...
    private func connectElevenLabsClient(apiKey: String) {
        elevenLabsClient = ElevenLabsLiveClient(
            apiKey: apiKey,
            modelID: modelID,
            language: language
        )
        SpeakLogger.network.info("Connecting to ElevenLabs streaming API")
        elevenLabsClient?.start(
    ...
    private func createAudioConverter(
        ...
        guard let targetFormat = AVAudioFormat(
            ...
            let error = iOSTranscriptionError.audioSessionFailed(
                NSError(domain: "ElevenLabsLiveTranscriber", code: -1,
        ...
        guard let converter = AVAudioConverter(from: nativeFormat, to: targetFormat) else {
            let error = iOSTranscriptionError.audioSessionFailed(
                NSError(domain: "ElevenLabsLiveTranscriber", code: -2,
    ...
    public func stop() async -> TranscriptionResult {
        guard isRunning else {
            return TranscriptionResult(
                ...
                confidence: nil,
                duration: 0,
                modelIdentifier: "elevenlabs/\(modelID)",
                cost: nil,
                rawPayload: nil,
                debugInfo: nil
            )
    ...
    public func cancel() {
        ...
        print("[ElevenLabsLiveTranscriber] Cancelled")
    }
    ...
    private func handleInterruption() {
        ...
        print("[ElevenLabsLiveTranscriber] Handling interruption")
