...
| Voice output | Engine only, with no UI or host wiring: a shared Deepgram path (canonical Aura and Flux voices, immutable pronunciation snapshot, one bounded cancellable request of up to 2,000 characters, validated lossless 24 kHz WAV) and `WindowsVoiceOutput`, which stores that audio in an exclusive private file and plays it through the native engine. It removes the file only once Windows allows it; a file still held, for example after a failed native release, stays owned within a bounded budget and is reported rather than claimed removed. Normal speed only | Native UI and host wiring, a real Deepgram receipt (including Flux linear16/WAV), physical speaker acceptance, long-text chunking, speed control, system voices, other providers and pronunciation editing |
