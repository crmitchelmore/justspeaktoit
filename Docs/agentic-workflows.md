...
- `issue-planning-ready-check` — adds or removes `planning:ready-for-dev` based on approvals
...
5. `Issue Planning - Ready Check` applies `planning:ready-for-dev` once all five approvals are present.

### Resetting or retesting planning

On the default branch, the issue open/comment events run automatically.
