...
| Voice output | Engine only, with no UI or host wiring: a shared Deepgram path (canonical Aura and Flux voices, immutable pronunciation snapshot, one bounded cancellable request of up to 2,000 characters, validated lossless 24 kHz WAV) and `WindowsVoiceOutput`, which stores that audio in an exclusive private file, plays it through the native engine and removes it after native release; normal speed only | Native UI and host wiring, a real Deepgram receipt (including Flux linear16/WAV), physical speaker acceptance, long-text chunking, speed control, system voices, other providers and pronunciation editing |
