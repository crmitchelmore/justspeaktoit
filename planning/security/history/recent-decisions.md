...
## 2026-04-08 — Issue #263: Not a PR (issue_comment on issue #263)
GitHub context shows `pull-request-number` is empty. #263 does not appear in the open PR list (only #267 and #128 are open). The `issue_comment` trigger fires on issue #263, which is an issue, not a PR. Per review protocol, took no action — the comment does not belong to a pull request.
